...
const skyGallery = [
  ...
  ["sky-tower.webp", "Fachada da Torre Lake Sky"],
] as const;
...
const mallGallery = [
  ...
  ["lake-mall-conference-hall.jpg", "Salão de conferências do Lake Mall"],
...
const loftGallery = [
  ...
  { src: "lake-loft-exclusive-pool.jpg", title: "Piscina exclusiva do Lake Loft", caption: "Uma área de lazer reservada para os moradores e hóspedes do Loft." },
...
export default async function PalmasLakeDevelopmentPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <div className="palmas-detail-page">
    ...
    {isLoft && <>
      ...
      <section className="palmas-sky-units palmas-loft-units" id="unidades" aria-labelledby="palmas-loft-units-title"><div className="palmas-sky-container"><div className="palmas-sky-section-heading"><p className="palmas-lake-kicker">Espelho de unidades</p><h2 id="palmas-loft-units-title">Confira as unidades por andar.</h2><p>Veja as áreas de cada loft e consulte a disponibilidade e as condições atualizadas no atendimento.</p></div><details className="palmas-loft-units-disclosure"><summary><span>Ver todas as 160 unidades</span><span className="palmas-loft-units-disclosure-hint">Clique para expandir</span></summary><div className="palmas-sky-table-wrap"><table><thead><tr><th>Pav.</th><th>Unidade</th><th>Área</th><th>Status</th></tr></thead><tbody>{loftUnits.map(([floor, unit, area]) => <tr key={unit}><td>{floor}</td><td>{unit}</td><td>{area}</td><td><span className="palmas-sky-status palmas-sky-status--consult">Consulte</span></td></tr>)}</tbody></table></div></details></div></section>
    ...
    {isSky && <>
      ...
      <section className="palmas-sky-plan" id="plantas-mansoes" aria-labelledby="palmas-sky-mansoes-title"><div className="palmas-sky-container palmas-sky-plan-grid"><div className="palmas-sky-plan-image"><Image src="/brand/palmas-lake/sky-mansoes-suspensas.webp" alt="Planta das mansões suspensas Lake Sky" fill sizes="(max-width: 900px) 100vw, 58vw" unoptimized /></div><div className="palmas-sky-plan-copy"><p className="palmas-lake-kicker">Planta Lake Sky · mansões suspensas</p><h2 id="palmas-sky-mansoes-title">Amplitude e privacidade em um apartamento por andar.</h2><p>A planta complementar apresenta a configuração das mansões suspensas: sala de estar integrada, sacada com 25 m², dependência e quatro suítes.</p><div className="palmas-sky-plan-tags"><span>331,29 m² privativos</span><span>4 suítes</span><span>4 vagas privativas</span></div></div></div></section>
