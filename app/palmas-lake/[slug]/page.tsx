...
export default async function PalmasLakeDevelopmentPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <div className="palmas-detail-page">
    <div className={`palmas-detail-hero${isGarden ? " palmas-detail-hero--garden" : ""}${isPark ? " palmas-detail-hero--park" : ""}${isMall ? " palmas-detail-hero--mall" : ""}`}><Image src={`/brand/palmas-lake/${item.image}.${isGarden || isPark ? "webp" : "jpg"}`} alt={`Imagem do ${item.name}`} fill priority sizes="100vw" /><div className="palmas-detail-overlay" /><div className="palmas-detail-top"><Link href="/palmas-lake" className="palmas-detail-back"><ArrowLeft size={16} /> Palmas Lake</Link><span>{isMall ? "Torre multifuncional" : item.group} · entrega {item.year}</span></div><div className="palmas-detail-hero-copy"><p>{isMall ? "Shopping conceito" : "Uma experiência Palmas Lake"}</p><h1>{item.name}</h1><h2>{item.title}</h2></div></div>
